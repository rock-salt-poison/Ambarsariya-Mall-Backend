...
//         // Collect headers in the first row
//         if (rowIndex === 0 && cell.effectiveValue) {
//           headerRow.push(cell.effectiveValue.stringValue || "");
//         }
//       });
//     });
...
//     const requests = [];
//     const lastRowIndex = lastRow;
//     const headers = headerRow.map((header) => header || "");
//     console.log("Headers: ", headers);

//     // Step 6: Apply formulas and pre-fill data based on queryData
//     const itemNoIndex = headers.indexOf("Item No");
//     const productNameIndex = headers.indexOf("Product Name");
//     const itemIdIndex = headers.indexOf("Item ID");
//     const noOfItemsIndex = headers.indexOf("No of Items");
...
//     // Pre-fill data based on queryData
//     const filledRows = queryData.map((data, index) => {
//       const row = Array(lastColumn).fill("");
//       if (productNameIndex !== -1) row[productNameIndex] = data.productName || "";
//       if (itemIdIndex !== -1) row[itemIdIndex] = data.itemId || "";
//       if (noOfItemsIndex !== -1) row[noOfItemsIndex] = data.noOfItems || "";
//       if (maxProductQuantityIndex !== -1)
//         row[maxProductQuantityIndex] = data.maxProductQuantity || "";
//       if (sellingPriceIndex !== -1) row[sellingPriceIndex] = data.sellingPrice || "";
//       if (itemPackageDimensionsIndex !== -1)
//         row[itemPackageDimensionsIndex] = data.itemPackageDimensions || "";
...
//       await sheets.spreadsheets.values.update({
//         spreadsheetId: userSheetId,
//         range: `${userSheetName}!A2`, // Start filling from the second row
//         valueInputOption: "USER_ENTERED",
//         requestBody: { values: filledRows },
//       });
//       console.log("Pre-filled data added successfully!");
//     }

//     // Step 7: Apply formatting and formulas to rows
//     adminSheet.data[0].rowData.forEach((row, rowIndex) => {
//       if (rowIndex >= lastRowIndex) return;
//       if (!row.values) return;
